...
export default function ReleaseNotesModal({ onClose }) {
  ...
  return (
    ...
    >
      <div style={{ background: 'var(--ink, #1a1410)', border: '1px solid rgba(176, 140, 63, 0.35)', borderRadius: '4px', maxWidth: '640px', width: '100%', maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0, 0, 0, 0.6)' }}>
        {/* Header */}
        <div style={{ padding: '1.6rem 2rem 1rem', borderBottom: '1px solid rgba(176, 140, 63, 0.2)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <div style={{ fontFamily: "'Special Elite', monospace", fontSize: '0.7rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--gilt)', marginBottom: '0.4rem' }}>
              {t('releaseNotes.changelog')}
            </div>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '1.7rem', color: 'var(--paper)', margin: 0 }}>
  ...
  return (
    <div style={{ paddingBottom: isLast ? 0 : '1.6rem', marginBottom: isLast ? 0 : '1.6rem', borderBottom: isLast ? 'none' : '1px solid rgba(176, 140, 63, 0.12)' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.7rem', marginBottom: '0.6rem', flexWrap: 'wrap' }}>
        <span style={{ fontFamily: "'Special Elite', monospace", fontSize: '0.85rem', letterSpacing: '0.1em', color: isCurrent ? 'var(--gilt-bright)' : 'var(--gilt)', background: isCurrent ? 'rgba(176, 140, 63, 0.15)' : 'transparent', border: isCurrent ? '1px solid rgba(176, 140, 63, 0.4)' : 'none', padding: isCurrent ? '0.15rem 0.5rem' : '0.15rem 0', borderRadius: '2px' }}>
          {release.version}
        </span>
        {isCurrent && (
          <span style={{ fontFamily: "'Special Elite', monospace", fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gilt-bright)' }}>
            {t('releaseNotes.currentBadge')}
          </span>
        )}
        {release.date && (
          <span style={{ fontSize: '0.75rem', color: 'var(--paper-aged)', opacity: 0.5, fontFamily: "'Special Elite', monospace" }}>
            {release.date}
          </span>
        )}
      </div>
      <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '1.2rem', color: 'var(--paper)', margin: '0 0 0.6rem', lineHeight: 1.3 }}>
        {title}
      </h3>
