...
export default function PlanView() {
    ...
    async function deletePlan() {
        ...
        if (plan._id) {
            ...
            await supabase.from('plans').delete().eq('id', plan._id);
        }
        // Remove from state
        const remaining = (state.plans || []).filter((p) => p._id !== plan._id);
        // setState isn't directly accessible here — use setCurrentPlan(null) which
        // triggers a reload; for now navigate away and let the next load reflect the deletion
        setCurrentPlan(null);
        go('dashboard');
    }
    ...
    return (
        <>
            ...
            <div style={{ marginBottom: '2rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-ghost" onClick={deletePlan}>
                    Delete this plan
                </button>
            </div>
