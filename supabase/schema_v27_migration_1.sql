...
--
-- NOTE: member avatars are ordered admins-first, then by user_id (stable but
-- arbitrary — book_club_members has no created_at/joined_at column to sort by
-- "who joined first"). Confirmed via runtime error against the live schema:
-- 42703 column m.created_at does not exist. If you add a joined_at column
-- later, swap `mm.user_id asc` for that instead.
